...
define("SISTEM_TMP",str_replace("\\","/",sys_get_temp_dir())."/ws-feeder");
@mkdir(SISTEM_TMP);
...
function makeExcel($data,$c,$title=''){
	...
	$objPHPExcel->getProperties()->setCreator("Mr Fedeer")->setLastModifiedBy("Mr Fedeer") ->setTitle("Mr Feeder Excel")->setSubject("Mr Feeder Excel Document")
		->setDescription("Document for Office 2007 XLSX, generated by Mr Feeder") ->setKeywords("office 2007 openxml php")->setCategory("Test result file");
...
function make_menu(){
	...
	echo "\r\n\r\n<div id=\"detail_indexing\" style=\"display:none\"><div align=\"center\">Tunggu Sedang re-index data</div><div align=\"center\" id=\"p_bar_indexing\"><img src=\"".PATH."/app/images/ajax-loader.gif\"></div><div align=\"center\" id=\"indexing_load\"></div></div>\r\n";
...
function kirim_data($url,$metod='get',&$data){
	...
	$cukis=json_decode(@file_get_contents(_FILE_COKIES),true); $cookies='';
	if(count($cukis)>0){
...
function set_coki($cookies,$set_cokis){
...
if($_GET['f']){
	...
	echo '</table>';
	$dirx=str_replace("/","\\\\",$dir);
	echo "<script>document.getElementById('ttitles').innerHTML='$dirx'; lokasi_dbf='".urlencode($dir)."';</script>";
	exit;
}
if($_GET['download']){
	$file=$_GET['download'];
...
if($_GET['reindex']){
	...
	if($_GET['reindex']=='1'){
		echo "<script>\r\npbar_progess=pbar_progess+1;\r\nvar itg=0;\r\n$(function() ".TANDA1."
			...
			".TANDA2.");
		".TANDA2.");\r\nfunction pbar_indexing()".TANDA1."
			itg=itg+1;
			var x=$s('#p_bar_indexing').text();
			$s('#p_bar_indexing').text(x+'.');
			if(itg==1)".TANDA1."
				$s('#p_bar_indexing').text('.');
			".TANDA2."
			setTimeout(\"pbar_indexing()\",1000);
			if(itg > 100)".TANDA1."
				itg=0;
				$s('#p_bar_indexing').text('.');
			".TANDA2."
		".TANDA2."
		\r\n/* setTimeout(\"pbar_indexing()\",500); */\r\n
...
$f=array();$f=explode("/", $_SERVER['SCRIPT_NAME']);$e=end($f);$f=explode("?",$_SERVER['REQUEST_URI']);$lx=$f[0]; $f=explode("/",$lx); $l=end($f);
